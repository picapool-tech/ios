...
class LiveOffersService {
  static Future<GetLiveOfferResponse> getLiveOffer(String liveOfferId, String accessToken)async{
  ...
  static Future<CreateLiveOfferResponse> createLiveOffer(CreateLiveOfferPayload offerPayload, String accessToken) async {
    try {
      await getAccessToken();
      final Dio dio = await getDio();
  ...
  static Future<List<LiveOffer>> getAllLiveOffers(String accessToken)async{
    try {
      ...
      final response = await dio.get(
        "${Constants.apiUrl}${Constants.getLiveOfferEndpoint}",
        options: Options(
          headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $accessToken'
      ...
      if (response.statusCode! < 300 && response.statusCode! >= 200) {
        ...
        return productList.map((liveOffer) => LiveOffer.fromJson(liveOffer)).toList();
